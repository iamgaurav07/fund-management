...
import { Plus, Filter, Search, Download, TrendingUp, DollarSign, PieChart } from 'lucide-react';
...
const FundsPage = () => {
  ...
  const [funds, setFunds] = useState<Fund[]>([
    ...
    },
    {
      ...
      carryPercent: 25,
      status: 'CLOSED',
      description: 'Series A and B investments in tech startups with disruptive potential.',
    ...
    },
    {
      ...
      carryPercent: 18,
      status: 'DRAFT',
      description: 'Renewable energy projects and green infrastructure investments.',
  ...
  useEffect(() => {
    ...
    if (statusFilter !== 'ALL') {
      result = result.filter(fund => fund.status === statusFilter);
    }
    setFilteredFunds(result);
  }, [funds, searchQuery, statusFilter]);
  ...
  const handleCreateFund = async (formData: FundFormData) => {
    ...
    const newFund: Fund = {
      ...
      ...formData,
      fundSize: Number(formData.fundSize),
      vintageYear: Number(formData.vintageYear),
      managementFeePercent: Number(formData.managementFeePercent),
      carryPercent: Number(formData.carryPercent),
  ...
  const totalAUM = funds.reduce((sum, fund) => sum + fund.fundSize, 0);
  const activeFunds = funds.filter(fund => fund.status === 'OPEN').length;
  const avgManagementFee = funds.reduce((sum, fund) => sum + fund.managementFeePercent, 0) / funds.length;

  const statusOptions = [
    { value: 'ALL', label: 'All Status', count: funds.length },
    { value: 'DRAFT', label: 'Draft', count: funds.filter(f => f.status === 'DRAFT').length },
    { value: 'OPEN', label: 'Open', count: funds.filter(f => f.status === 'OPEN').length },
    { value: 'CLOSED', label: 'Closed', count: funds.filter(f => f.status === 'CLOSED').length },
    { value: 'LIQUIDATED', label: 'Liquidated', count: funds.filter(f => f.status === 'LIQUIDATED').length },
  ];
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 py-6">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">Fund Management</h1>
                  <p className="text-gray-600 mt-1">Manage your investment funds portfolio</p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900 mt-2">{formatCurrency(totalAUM)}</p>
              </div>
              <DollarSign className="w-10 h-10 text-primary-600 opacity-80" />
            </div>
            <p className="text-xs text-gray-500 mt-3">Across {funds.length} funds</p>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">Active Funds</p>
                <p className="text-2xl font-bold text-gray-900 mt-2">{activeFunds}</p>
              </div>
              <TrendingUp className="w-10 h-10 text-emerald-600 opacity-80" />
            </div>
            <p className="text-xs text-gray-500 mt-3">{((activeFunds / funds.length) * 100).toFixed(0)}% of portfolio</p>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">Avg. Management Fee</p>
                <p className="text-2xl font-bold text-gray-900 mt-2">{avgManagementFee.toFixed(2)}%</p>
              </div>
                    ...
                    className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                      statusFilter === option.value
                        ? option.value === 'ALL' ? 'bg-gray-900 text-white' :
                          option.value === 'DRAFT' ? 'bg-blue-100 text-blue-800 border border-blue-200' :
                          option.value === 'OPEN' ? 'bg-green-100 text-green-800 border border-green-200' :
                          option.value === 'CLOSED' ? 'bg-gray-100 text-gray-800 border border-gray-200' :
